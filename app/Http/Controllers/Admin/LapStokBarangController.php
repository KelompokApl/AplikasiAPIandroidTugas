    ...
    public function print(Request $request)
    {
        $data['data'] = BarangModel::leftJoin('tbl_jenisbarang', 'tbl_jenisbarang.jenisbarang_id', '=', 'tbl_barang.jenisbarang_id')
            ...
            ->where(function ($query) use ($request) {
                if ($request->barang != '') {
                    $query->where('tbl_barang.barang_nama', 'LIKE', '%' . $request->barang . '%');
                    $query->orWhere('tbl_barang.barang_kode', 'LIKE', '%' . $request->barang . '%');
                }
            })
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            $data = BarangModel::leftJoin('tbl_jenisbarang', 'tbl_jenisbarang.jenisbarang_id', '=', 'tbl_barang.jenisbarang_id')
                ...
                ->where(function ($query) use ($request) {
                    if ($request->nmbarang != '') {
                        $query->where('tbl_barang.barang_nama', 'LIKE', '%' . $request->nmbarang . '%');
                        $query->orWhere('tbl_barang.barang_kode', 'LIKE', '%' . $request->nmbarang . '%');
                    }
                })
                ->orderBy('barang_id', 'DESC')
                ->get();
